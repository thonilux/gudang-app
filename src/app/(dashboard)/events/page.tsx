        ...
        {overview.events.length === 0 ? (
          ...
          <>
            ...
            <div className="md:hidden mt-5 space-y-4">
              {overview.events.map((event) => (
                <div key={event.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-soft space-y-3">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <Link prefetch={false}
                        href={`/events/${event.id}?tab=detail`}
                      ...
                      <h4 className="text-sm font-medium text-slate-700 mt-1">{event.name}</h4>
                      <p className="text-xs text-slate-500 mt-0.5">{event.clientName || "Tanpa klien"}</p>
                    </div>
                    <span
                      className={`inline-flex rounded-full border px-2.5 py-0.5 text-[10px] font-semibold ${getStatusTone(event.status)}`}
                    >
                      {statusLabel(event.status)}
                    </span>
                  </div>
                  ...
                  <div className="border-t border-slate-100 pt-3 text-xs text-slate-600 space-y-1">
                    <p><span className="font-semibold text-slate-700">Lokasi:</span> {event.venueName || "-"}</p>
                    <p><span className="font-semibold text-slate-700">Mulai:</span> {formatDate(event.startAt)}</p>
                    <p><span className="font-semibold text-slate-700">Selesai:</span> {formatDate(event.endAt)}</p>
                  </div>

                  <div className="border-t border-slate-100 pt-3 flex items-center justify-between gap-2 text-xs">
                    <div>
                      <p className="font-semibold text-slate-800">{event.equipmentCount} item</p>
                      <p className="text-[10px] text-slate-500 mt-0.5">Booking: {event.bookingCount} · Load: {event.loadingCount} · Return: {event.returnCount}</p>
                    </div>
